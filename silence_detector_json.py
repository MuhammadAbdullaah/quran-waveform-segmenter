...
def compute_rms(y: np.ndarray, sr: int, win_ms: int = 102) -> Tuple[np.ndarray, np.ndarray, int]:
    """
    Compute non-overlapping RMS frames similar to the Streamlit tool:
    - hop = sr * (win_ms/1000)
    - rms per frame = sqrt(mean(frame*frame))
    - normalize by max RMS
    Returns normalized rms, times (seconds), hop (samples)
    """
...
def detect_silences_energy(rms_norm: np.ndarray, times: np.ndarray, hop: int, sr: int,
                           threshold: float = 0.16, min_sil_ms: int = 800, merge_ms: int = 400) -> List[Dict]:
    """
    Silence detection logic matching Streamlit:
    - Start silence when frame value < threshold
    - End silence when frame value >= threshold
    - Only keep silences with duration >= min_sil_ms
    - Merge silences whose gap <= merge_ms
    Returns list of silences with start_idx,end_idx,start_time,end_time (seconds)
    """
    if rms_norm.size == 0:
        return []

    # Convert ms thresholds to frames counts
    ...
    for i, value in enumerate(rms_norm):
        if value < threshold and not in_silence:
            ...
            in_silence = False
            silence_end = i - 1
            silence_duration_ms = (silence_end - silence_start + 1) * (hop / float(sr)) * 1000.0
            if silence_duration_ms >= min_sil_ms:
                silences.append({
                    "start_idx": int(silence_start),
                    "end_idx": int(silence_end),
                    "start_time": float(silence_start * (hop / float(sr))),
                    "end_time": float((silence_end + 1) * (hop / float(sr)))
                })

    # Silence continuing to end
    if in_silence:
    ...
    # Merge nearby silences (gap measured in frames)
    merged = []
    current = silences[0]
    for next_s in silences[1:]:
        gap_frames = next_s["start_idx"] - current["end_idx"]
        if gap_frames <= merge_frames:
            # extend current silence
            current["end_idx"] = next_s["end_idx"]
            current["end_time"] = next_s["end_time"]
        else:
...
def format_silences_for_output_ms(silences: List[Dict]) -> List[Dict]:
    """
    Convert silence times (seconds) to milliseconds and produce array-only JSON objects.
    Fields: number, start_time_ms, end_time_ms, duration_ms
    """
    formatted = []
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=0.18, help="RMS threshold (0..1). Default 0.16")
    parser.add_argument("--min_silence", type=int, default=400, help="Minimum silence duration in ms. Default 800")
    parser.add_argument("--merge_gap", type=int, default=130, help="Merge gap threshold in ms. Default 400")
    parser.add_argument("--window", type=int, default=102, help="Window size in ms for RMS frames. Default 102")
